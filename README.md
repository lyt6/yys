...
- 重启会延续上次轮询间隔；验证、限流、拒绝访问等状态默认冷却 15 分钟，反复重启不会提前请求。
...
- 上一轮若是手机验证、登录失效、429、401/403 或明确业务错误，默认从失败完成时间起冷却 900 秒。
...
| `CBG_RESTART_COOLDOWN_SECONDS` | `900` | 验证、限流、拒绝访问等终止状态的重启冷却，最低 60 秒 |
