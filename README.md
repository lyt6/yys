...
自动登录会按“阅读并同意”文字定位当前可见协议行，点击协议组件的非链接文字区域以同步组件内部状态，并在短暂等待后复查 checkbox 是否仍保持勾选；只修改隐藏 input 的瞬时 `checked` 不再视为成功。若提交后出现“需要同意相关条款”等提示，会强制重新触发协议组件并仅重试提交一次。普通账号密码或协议提交失败只使用正常轮询间隔，不会进入 15 分钟风控冷却。更新这部分代码后需要停止并重新启动现有采集进程，运行中的 Python 不会自动加载新逻辑。
