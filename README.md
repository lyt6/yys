...
自动登录会按“已阅读并同意”文字定位当前可见协议行，优先点击左侧可见方框，再校验内部 checkbox/自定义控件状态；若提交后出现“需要同意相关条款”等提示，会重新点击并仅重试提交一次。普通账号密码或协议提交失败只使用正常轮询间隔，不会进入 15 分钟风控冷却。更新这部分代码后需要停止并重新启动现有采集进程，运行中的 Python 不会自动加载新逻辑。
