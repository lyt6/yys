...
- 页面加载前将官网客户端切换到其自带的普通 `query` 查询分支；不会把
  `recommend.py` 推荐流的结束误认为全站覆盖完成。
- 预览页分别显示“已累计采集”和接口报告总量，避免把 SQLite 历史累计数误解为
  当前官方挂牌总数。
...
当前 `/cgi/api/query` 与 `/cgi-bin/recommend.py` 的 `result[]` 会解析 `eid`、`game_ordersn`、`equipid`、`desc_sumup_short`、`format_equip_name` 和 `other_info` 等字段；这些角色列表接口的价格以分返回，列表及 SQLite 的 `price` 转成元并保留两位小数，原始分值仍保存在 `detail_json`。其他接口的价格单位保持原样。
...
- 默认在页面脚本执行前设置官网已有的 `open_recommd=false` 客户端开关，让页面走
  `/cgi/api/query` 普通查询分支；请求仍由已登录浏览器页面自身发出。
- 普通扫描每 60 秒调度一次，最多滚动 20 轮。
- 深度扫描默认每 3600 秒执行一次，最多滚动 1000 轮，可覆盖接口当前报告的
  约 10000 条、每页约 15 条数据。
- 每轮滚动后按“本轮累计观察到的唯一 ID 数量”判断是否加载了下一批数据。
- 连续 3 轮没有观察到更多唯一 ID 时，以 `termination_reason=idle` 停止，但不会宣称已经扫描到底。
- 只有普通查询接口明确返回 `has_more=false`、`is_end=true`、
  `paging.is_last_page=true`，本轮唯一 ID 达到接口 `total_num`，或普通查询页面出现明确
  的“没有更多”标记时，`scan_complete=true`。
- 如果官网改版导致普通查询开关失效，采集器仍可解析推荐流，但会明确标记为
  `recommendation_fallback` 且 `scan_complete=false`。

这能覆盖“前几页都是已知项目、较深位置才有批量更新”的情况，因为停止条件不再使用“相对于旧快照的变更数”。

普通查询当前按官网默认的最新上架顺序分页，并提供 `total_num` 与
`paging.is_last_page`。分钟级增量扫描覆盖靠前的新上架或更新数据，定时深扫负责重新覆盖
完整查询范围。当前没有官方 `since` 增量游标，不能凭空追加页面未使用的时间参数。

这里的“全部数据”始终限定于当前目标 URL、页面筛选条件、账号权限、官方查询上限和
最大滚动轮数所能加载的记录；接口当前报告 10000 条不等于平台永远只有 10000 条。
...
| `CBG_MAX_PAGES` | `1000` | 深度扫描最大滚动轮数；10000 条、每页 15 条约需 667 轮 |
| `CBG_SCROLL_DELAY_MS` | `3000` | 滚动后等待响应时间，最低 1000ms |
| `CBG_IDLE_ROUNDS` | `3` | 连续无新增观察的停止阈值 |
| `CBG_FORCE_FULL_QUERY` | `true` | 使用官网普通 query 分支；`false` 恢复官网推荐流 |
...
- 没有官方 `since` 增量游标，有限深度的分钟扫描无法证明覆盖所有历史位置；依靠定时深扫补全。
- 普通查询报告的 `total_num` 可能是官方展示上限，程序只能覆盖当前账号与筛选条件允许读取的范围。
